				...
				<div className="flex flex-wrap justify-center gap-8 sm:gap-12 md:gap-16">
					{brands.map((brand, index) => (
						<div key={index} className="flex items-center justify-center w-1/2 sm:w-1/3 md:w-1/4 lg:w-auto">
							{brand.logo && <Image className={`w-[100px] h-[24px] sm:h-[28px] md:h-[32px] grayscale hover:grayscale-0 transition-all duration-300 ${brand.name === "Bayliner" ? "invert" : ""}`} src={brand.logo} alt={`${brand.name} logo`} width={brand.width} height={brand.height} loading="lazy" />}
						</div>
					))}
				</div>
			</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
					...
					<Card className="overflow-hidden transition-all duration-300 hover:shadow-xl">
						<div className="md:flex h-full">
							<div className="md:w-2/5 relative">
								<Image alt="Tige Boat - Premium wakesurfing boat by Impact Marine Group" src="https://2gqfqtxkmitzixum.public.blob.vercel-storage.com/service-department-64nOGoKYBK1mgMV0VLU49TnYFlbuw6.webp" fill className="object-cover" loading="lazy" />
							</div>
							<div className="md:w-3/5">
								<CardHeader className="bg-muted">
									<CardTitle className="text-2xl font-bold flex items-center">
...
function SocialSection() {
	...
	useEffect(() => {
		setCommentCount(Math.floor(Math.random() * 200) + 50);
	}, []);
					...
					{[...Array(6)].map((_, index) => (
						<Card key={index} className="overflow-hidden border-2 border-muted hover:border-primary transition-colors duration-300 shadow-lg border-gray-200">
							...
							<div className="relative group">
								<Image src="https://2gqfqtxkmitzixum.public.blob.vercel-storage.com/service-department-64nOGoKYBK1mgMV0VLU49TnYFlbuw6.webp" alt={`Beautiful boat showcased by Impact Marine Group on Instagram`} width={400} height={400} className="transition-transform duration-300 group-hover:scale-105 object-cover" loading="lazy" />
